...
__global__ void blurKernel(unsigned char* in, unsigned char* out, int w, int h){
	...
	if(col < w && row < h){
	        // Each thread copies its element from global to shared memory
	        in_shared[threadIdx.y][threadIdx.x] = in[row*w + col];
		__syncthreads();
		...
		for(int blurRow = -BLUR_SIZE; blurRow <= BLUR_SIZE; ++blurRow){
			for(int blurCol = -BLUR_SIZE; blurCol <= BLUR_SIZE; ++blurCol){
				...
				if(curRow >= 0 && curRow < h && curCol >= 0 && curCol < w){
					// We need to check if the element in[curRow][curCol]
					// is one of those managed by this block (and then we
					// find the data in shared memory), or managed by a different
					// block (and then we need to get the data from global memory).
					// To do so, we can just divide the curRow by the BLOCK_SIZE.
					// If that's different from row/BLOCK_SIZE, then this thread
					// and the element it wants to read now are in two different
					// blocks, and it is read from global memory (the same for curCol).
					if(curRow / BLOCK_SIZE != row / BLOCK_SIZE || curCol / BLOCK_SIZE != col / BLOCK_SIZE){
                                            pixVal += in[curRow*w + curCol];
					}else{
					    int curRowShared = threadIdx.y + blurRow;
					    int curColShared = threadIdx.x + blurCol;
					    pixVal += in_shared[curRowShared][curColShared];
					}
					pixels++;
				}
			}
